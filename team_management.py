...
def init_load_user(interaction: discord.Interaction) -> UserData:
    ...
    try:
        ...
        user, result = UserData.create_new_entry(
            id=interaction.user.id, balance=config().starting_budget
        )  # 새 사용자 DB에 저장
...
class TeamManagement(commands.Cog):
    ...
    @app_commands.command(
        name="선수등록", description="해당 포지션의 선수를 내 팀에 등록합니다."
    )
    ...
    async def purchase_player(
        ...
        if not config().is_registration_active:
            await interaction.response.send_message(
                "현재 선수 등록이 비활성화되어 있습니다.", ephemeral=True
            )
            return
        ...
        try:
            player_data = PlayerData.load_from_db(
                player_name=name
            )  # 이름으로 선수 데이터 로드
        except ValueError:
            logger.debug(f"{name} 선수는 올바른 이름이 아닙니다.")
            await interaction.response.send_message(
                f"{name} 선수는 올바른 이름이 아닙니다.", ephemeral=True
            )
            return
        ...
        if not player_data.purchasable:
            await interaction.response.send_message(
                f"{name}은(는) 거래가 제한된 상태입니다.", ephemeral=True
            )
            return
        ...
        if player_data.position != position:
            await interaction.response.send_message(
                f"{name} 선수의 포지션은 {player_data.position}입니다. 올바른 포지션을 입력해주세요.",
                ephemeral=True,
            )
            return
        ...
        if getattr(user, pos_alias[player_data.position]) is not None:
            await interaction.response.send_message(
                f"{position} 포지션에는 이미 선수가 등록되어 있습니다.", ephemeral=True
            )
            return
        ...
        if user.balance < player_data.value:
            await interaction.response.send_message(
                f"골드가 부족합니다. 현재 예산: {user.balance}골드", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"{name} 선수가 {position} 포지션에 등록되었습니다. 현재 예산: {user.balance} 골드",
            ephemeral=True,
        )

    @app_commands.command(
        name="선수판매", description="해당 포지션의 선수를 판매합니다."
    )
    ...
    async def sell_player(
        ...
        if not config().is_sale_active:
            await interaction.response.send_message(
                "현재 선수 판매가 비활성화되어 있습니다.", ephemeral=True
            )
            return
        ...
        if position == "all":
            ...
            for pos in ["탑", "정글", "미드", "원딜", "서폿"]:
                ...
                if player is not None:
                    ...
                    logger.info(
                        f"{player.name} 선수가 {pos} 포지션에서 판매되었습니다."
                    )

            await interaction.response.send_message(
                f"모든 선수가 판매되었습니다. {total_gold} 골드를 얻었습니다. 현재 예산: {user.balance} 골드",
                ephemeral=True,
            )
            ...
            if player is None:
                await interaction.response.send_message(
                    f"{position} 포지션에 등록된 선수가 없습니다.", ephemeral=True
                )
                return

            # 선수 판매
            player_cost = round(
                player.value * (1 - config().sale_charge)
            )  # 선수 비용 계산
            if user.sell_player(pos_alias[position]):  # 선수 판매
                await interaction.response.send_message(
                    f"{player.name} 선수가 판매되었습니다. {player_cost} 골드를 얻었습니다. 현재 예산: {user.balance} 골드",
                    ephemeral=True,
                )
            else:
                await interaction.response.send_message(
                    f"{player.name} 선수를 판매할 수 없습니다.", ephemeral=True
                )

        else:
            await interaction.response.send_message(
                f"{position}은(는) 올바른 포지션이 아닙니다.", ephemeral=True
            )
    ...
    async def myteam(self, interaction: discord.Interaction):
        if interaction.channel.id not in config().community_channel_id:
            await interaction.response.send_message(
                "이 명령어는 지정된 채널에서만 사용할 수 있습니다.", ephemeral=False
            )
            return
        ...
        if not user:
            await interaction.response.send_message(
                "팀 정보가 없습니다. 먼저 선수를 등록해주세요.", ephemeral=True
            )
            return
        ...
        for position, player in team_info.items():
            if player:  # 선수가 등록되어 있는 경우
                team_display += f"- {position}: {player.name} (가치: {player.value}{ f"(+{user.get_single_team_bonus()})" if single_team else "" } 골드)\n"
