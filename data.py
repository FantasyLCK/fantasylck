...
class PlayerData:
    ...
    def offset(self, new_offset: int) -> int:
        players_collection().update_one(
            {"player_id": self.id}, {"$set": {"offset": new_offset}}, upsert=True
        )
    ...
    @property
    def value(self):
        team_placement_bonus = Fraction(
            100 + self.team.get_team_placement_bonus_ratio(), 100
        )
        return round(
            (
                get_player_cost(self.tier)
                + self.offset
                + (config().pog_bonus * self.pog_stacks)
            )
            * team_placement_bonus
        )
    ...
    def pog_stacks(self) -> int:
        return min(
            self.__retrieve_db()["pog_stacks"],
            config().pog_stack_bound[self.tier.lower()],
        )
    ...
    def pog_stacks(self, stack: int):
        ...
        players_collection().update_one(
            {"player_id": self.id}, {"$set": {"pog_stacks": stack}}, upsert=True
        )
    ...
    def create_new_entry(
        id, name, position, team, tier, trait_weight
...
class TeamData:
    ...
    def load_from_db(team_id: int = -1, name: str = None) -> "TeamData":
        ...
        if data is not None:
            ...
            raise ValueError(
                f'Team with (id = { team_id } / name = "{ name }") not found in database.'
            )
    ...
    def placement(self, new_placement: int):
        ...
        teams_collection().update_one(
            {"id": self.__id}, {"$set": {"placement": new_placement}}, upsert=True
        )
...
class UserData:
    ...
    def top_id(self, id: int):
        ...
        users_collection().update_one(
            {"discord_id": self.__discord_id}, {"$set": {"top": id}}, upsert=True
        )
    ...
    def jgl_id(self, id: int):
        if self.jgl_id >= 0 and id >= 0:
            raise AttributeError
        users_collection().update_one(
            {"discord_id": self.__discord_id}, {"$set": {"jgl": id}}, upsert=True
        )
    ...
    def mid_id(self, id: int):
        if self.mid_id >= 0 and id >= 0:
            raise AttributeError
        users_collection().update_one(
            {"discord_id": self.__discord_id}, {"$set": {"mid": id}}, upsert=True
        )
    ...
    def adc_id(self, id: int):
        if self.adc_id >= 0 and id >= 0:
            raise AttributeError
        users_collection().update_one(
            {"discord_id": self.__discord_id}, {"$set": {"adc": id}}, upsert=True
        )
    ...
    def sup_id(self, id: int):
        if self.sup_id >= 0 and id >= 0:
            raise AttributeError
        users_collection().update_one(
            {"discord_id": self.__discord_id}, {"$set": {"sup": id}}, upsert=True
        )
    ...
    def update_balance(self, amount: int):
        ...
        users_collection().update_one(
            ...
            {"$set": {"balance": self.balance + amount}},
            upsert=True,
        )
    ...
    def has_full_roster(self):
        return (
            users_full_roster_collection().find_one({"discord_id": self.discord_id})
            is not None
        )
...
def load_and_save_data():
    ...
    try:
        with open("players.json", "r", encoding="utf-8") as f:
            ...
            print(
                "Type of players_data:", type(players_data)
            )  # Debug: type of the loaded data
            print(
                "Loaded players data:", players_data
    ...
    if isinstance(players_data, list):
        for player_info in players_data:
            ...
            if result:
                ...
                logger.info(
                    f"Player {player_info['name']} already exists in the database."
                )
        ...
        print(
            "Players data is neither a list nor a dictionary, skipping database update."
        )
    ...
    try:
        with open("users.json", "r", encoding="utf-8") as f:
            ...
            print(
                "Type of users_data:", type(users_data)
            )  # Debug: type of the loaded data
            print(
                "Loaded users data:", users_data
            )  # Debug print to inspect the structure
    except Exception as e:
    ...
    for user_info in users_data:
        ...
        if result:
            ...
            logger.info(
                f"User {user_info['discord_id']} already exists in the database."
            )
